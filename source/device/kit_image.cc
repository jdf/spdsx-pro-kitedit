...
std::vector<KitRecord> ParseKits(const Bytes& clean_image) {
  ...
  for (int i = 0; i < kBankKitCount; ++i) {
    ...
    // and the layer-table block differ.
    auto parse_object = [&](size_t block, int layer_block) {
      PadDeviceParams pp;
      ...
      pp.link_send = clean_image[block + kLinkSendOffset];
      pp.link_receive = clean_image[block + kLinkReceiveOffset];
