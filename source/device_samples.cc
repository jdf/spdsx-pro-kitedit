...
DeviceSamplePanel::DeviceSamplePanel(const DeviceModel& device,
    ...
    : device_(device)
    , settings_(settings) {
  filter_.setTextToShowWhenEmpty(juce::String::fromUTF8("filter\xe2\x80\xa6"),
                                 kMeta);
...
void DeviceSamplePanel::paintListBoxItem(
  ...
  g.drawText(
      juce::String(rec->wavename), area, juce::Justification::centredLeft);
}
