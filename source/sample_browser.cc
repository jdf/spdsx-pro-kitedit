...
SampleBrowser::SampleBrowser(juce::ApplicationProperties& settings)
  ...
  contents_ = std::make_unique<juce::DirectoryContentsList>(filter_.get(),
                                                            scan_thread_);
  ...
  tree_->setColour(juce::DirectoryContentsDisplayComponent::textColourId,
                   kTreeText);
  tree_->setColour(juce::DirectoryContentsDisplayComponent::highlightColourId,
  ...
  auto saved =
      juce::File(settings_.getUserSettings()->getValue("sampleBrowserRoot"));
...
void SampleBrowser::selectionChanged() {
  if (restoring_
      || !settings_.getUserSettings()->getBoolValue("autoplayBrowsing",
                                                    false)) {
    return;
  }
...
void SampleBrowser::SetRoot(const juce::File& root, bool persist) {
  ...
  if (persist) {
    settings_.getUserSettings()->setValue("sampleBrowserRoot",
...
void SampleBrowser::ChooseRoot() {
  chooser_ = std::make_unique<juce::FileChooser>("Choose the sample folder",
                                                 contents_->getDirectory());
...
void SampleBrowser::resized() {
  ...
  root_button_.setBounds(header.removeFromRight(kHeaderHeight).reduced(4));
