...
juce::Path PencilPath() {
  ...
  p.addQuadrilateral(0.28f, 0.62f, 0.62f, 0.28f, 0.74f, 0.40f, 0.40f, 0.74f);
...
void KitChooser::OpenMenu() {
  ...
  for (int i = 0; i < kit_count_; ++i) {
    menu.addItem(
        i + 1,
        juce::String(i + 1) + "  " + (kit_name ? kit_name(i) : juce::String()),
        true,
        i == current_);
  }
...
void KitChooser::paint(juce::Graphics& g) {
  ...
  g.drawText(
      juce::String(current_ + 1), number_area_, juce::Justification::centred);
  const auto tri = triangle_area_.toFloat();
  juce::Path p;
  p.addTriangle(tri.getX(),
                tri.getY(),
                tri.getRight(),
                tri.getY(),
                tri.getCentreX(),
...
void KitChooser::resized() {
  ...
  triangle_area_ = inner.removeFromRight(16).withSizeKeepingCentre(10, 6);
