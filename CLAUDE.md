...
- `kit_document.{h,cc}` — `KitDocument : FileBasedDocument`, versioned JSON `.kit` (`KitFormat` enum, current v4 = per-pad `mode`/`fadePoint`/`fadeEnd` + `dynamics`/`dynamicsCurve`/`triggerReserve`; `pads` array; absolute sample paths; legacy flat-`slots` still loads; loading clears undo history). NOTE: FileBasedDocument's sync save/load (saveAs/loadFrom) are compiled out in JUCE 8 (no modal loops) — selftests go through temporary hooks calling the protected saveDocument/loadDocument.
